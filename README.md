...
- Next-Day Order Planner: converts strategy-level buy/sell/stop references into more reachable next-session buy/sell ranges, tactical stop, hard stop, and touch-probability labels for current holdings. It integrates 15m / 1h / 1d SMC signals into SMC confidence, buy urgency, sell urgency, and a green/red/blue priority heatmap rendered with pandas Styler / Streamlit dataframe (so raw `<tr>` / `<td>` table source cannot leak into the UI), then links each row to a swing trading technical chart with candlesticks, Bollinger Bands, RSI, MACD, volume, candlestick-pattern markers, order zones, smartmoneyconcepts-first SMC overlays for FVG/IFVG, Order Blocks, Liquidity, Swing High/Low, BOS/ChoCH, fallback internal rules, icon-prefixed legend glossary, and UKF-style denoised momentum. If the Strategy Workbench has been run, the planner adds final source/side/strategy/edge fields and uses the workbench's final buy/sell/stop/take-profit ranges in the table, heatmap, and row-linked chart. It is research support only and never places orders.
- Next-Day Strategy Workbench: button-triggered actionable strategy lab after the order planner. Users first choose one ticker or all tickers, set risk tolerance (default 10%), choose a common 1/5/10/15/30-day holding basis, select strategy families (Bollinger, SMC, UKF momentum, KD/MACD, SHAP-factor proxy), choose a trailing backtest range, then run a per-ticker strategy suitability backtest and receive win rate, Profit Factor, drawdown, strategy edge score, BUY/SELL/WAIT side, urgency score, and final buy/sell/stop zones. The workbench also includes a ticker dropdown and strategy selector that render strategy buy/sell markers, per-trade vertical entry/exit lines, green/red profit/loss connectors, strategy buy/sell zones, optional SMC Order Block/Liquidity overlays, equity curve, drawdown curve, and per-strategy performance metrics on one chart. The suitability bar colors mean high/mixed/weak strategy fit, not SHAP positive/negative contribution. The sidebar `Personal trading preferences` section records no-day-trading, max orders per stock per day, default shares/lots per order, and usual holding days so backtests better match manual trading habits. Its recommendations are merged back into the Next-Day Order Planner as the final manual order ranges.
...
- Professional Research Center: combines Research Radar, Strategy Tester, Factor Explorer, Training Data Studio, and Risk & Correlation Monitor. Like a TradingView + backtest-platform research workflow, users first inspect the overview, then strategy tests, factor rankings, model contribution, training data, and correlation risk. Color semantics plus AUC / Profit Factor / max-drawdown explanations are written in plain language, and expensive analyses remain button-triggered so the dashboard stays responsive.
...
The left sidebar now keeps only global settings: data source, ticker universe, history period, candlestick interval, decision horizon, personal trading preferences, CSV upload, and manual refresh. Backtest / Smart Tuning parameters live in the Backtest tab; factor-research parameters live in the Factor research tab; price-chart volume visibility lives in the Price chart tab. This keeps the sidebar from becoming a long mixed control panel.
...
8. Open Next-Day Order Planner to review next-session buy/sell ranges, tactical stop, hard stop, touch probability, final strategy-backed order ranges, and the row-linked swing trading chart with Bollinger/RSI/MACD/volume/candlestick-pattern/UKF momentum context.
9. Open Next-Day Strategy Workbench to choose ticker scope, holding days, risk tolerance, strategy families, and backtest range, then run a button-triggered strategy suitability check; use the strategy visualization dropdown to inspect buy/sell markers, equity, drawdown, and optional SMC overlays for one ticker; return to Next-Day Order Planner to see the final buy/sell/stop zone merged into the main order table.
10. Check Backtest for win rate, maximum drawdown, stop-loss hit rate, and cumulative return.
11. Check Factor research to compare which 1/3/5/10-day horizon has stronger signal.
12. When deeper explanation is needed, run Attribution or Factor research from the button-triggered sections.
13. Use Stock relationships to check whether selected stocks share the same risk exposure.
...
- order_strategy_workbench.py: button-triggered Next-Day Strategy Workbench; compares Bollinger, SMC, UKF momentum, KD/MACD, and SHAP-factor-proxy strategy families for selected tickers, holding days, risk tolerance, and backtest range, then ranks BUY/SELL/WAIT urgency and final order zones.
