...
- Research and Training Data Center: button-triggered generation of one daily row per ticker with OHLCV, RSI/MACD/KD/Bollinger/volume, UKF momentum, candlestick patterns, SMC FVG/Order Block/Liquidity/BOS/ChoCH flags, and future 1/3/5/10-day return/up-down targets. `target_available_Nd` marks whether a row truly has a future answer, so the last N rows are not accidentally used as labels. The page ranks the top related columns so the dataset can become the foundation for a future 3-day trend AI model.
