...
_TRANSLATIONS.update(
    {
        ...
        "智能交易視覺中心": {"en": "Trade Vision Center", "ja": "スマート取引ビジョンセンター", "ko": "스마트 거래 비전 센터"},
        "整合 K 線、市場結構、支撐壓力、交易計畫、MTF 矩陣與綜合分數。此頁是研究輔助，不自動下單。": {"en": "Integrates candlesticks, market structure, support/resistance, trade plans, MTF matrix, and composite score. This page is for research support only and never places trades.", "ja": "ローソク足、市場構造、支持/抵抗、取引計画、MTF 行列、総合スコアを統合します。このページは研究支援のみで自動注文は行いません。", "ko": "캔들, 시장 구조, 지지/저항, 거래 계획, MTF 매트릭스, 종합 점수를 통합합니다. 이 페이지는 연구 보조용이며 자동 주문을 하지 않습니다."},
        "選擇 ticker": {"en": "Select ticker", "ja": "銘柄を選択", "ko": "종목 선택"},
        "顯示成交量": {"en": "Show volume", "ja": "出来高を表示", "ko": "거래량 표시"},
        "顯示 BOS / ChoCH": {"en": "Show BOS / ChoCH", "ja": "BOS / ChoCH を表示", "ko": "BOS / ChoCH 표시"},
        "顯示支撐壓力區": {"en": "Show support/resistance zones", "ja": "支持/抵抗ゾーンを表示", "ko": "지지/저항 구간 표시"},
        "顯示 Entry / SL / TP": {"en": "Show Entry / SL / TP", "ja": "Entry / SL / TP を表示", "ko": "Entry / SL / TP 표시"},
        "資料不足以建立 Trade Vision Center；請拉長歷史區間或改用日 K。": {"en": "Not enough data to build the Trade Vision Center. Extend the history range or use daily candles.", "ja": "Trade Vision Center を作成するデータが不足しています。履歴期間を延ばすか日足を使用してください。", "ko": "Trade Vision Center를 만들 데이터가 부족합니다. 기간을 늘리거나 일봉을 사용하세요."},
        "研究輔助，不自動下單。實際下單前請自行確認券商報價、流動性、稅費與個人風險承受度。": {"en": "Research support only; no automatic orders. Before placing any order, verify broker quotes, liquidity, taxes/fees, and your personal risk tolerance.", "ja": "研究支援のみで自動注文は行いません。実際に注文する前に、証券会社の価格、流動性、税金/手数料、個人のリスク許容度を確認してください。", "ko": "연구 보조용이며 자동 주문은 하지 않습니다. 실제 주문 전에는 브로커 시세, 유동성, 세금/수수료, 개인 위험 감내도를 확인하세요."},
        "項目": {"en": "Item", "ja": "項目", "ko": "항목"},
        "分數": {"en": "Score", "ja": "スコア", "ko": "점수"},
        "時間框架": {"en": "Timeframe", "ja": "時間軸", "ko": "시간 프레임"},
        "趨勢狀態": {"en": "Trend state", "ja": "トレンド状態", "ko": "추세 상태"},
        "動能分數": {"en": "Momentum score", "ja": "モメンタムスコア", "ko": "모멘텀 점수"},
        "量能分數": {"en": "Volume score", "ja": "出来高スコア", "ko": "거래량 점수"},
        "波動分數": {"en": "Volatility score", "ja": "ボラティリティスコア", "ko": "변동성 점수"},
        "訊號強度": {"en": "Signal strength", "ja": "シグナル強度", "ko": "신호 강도"},
...
_NEXT_DAY_ORDER_TRANSLATIONS = {
    "隔日掛單計畫": {"en": "Next-Day Order Planner", "ja": "翌日注文プランナー", "ko": "익일 주문 계획"},
    "把策略級買進 / 停利 / 停損價，轉換成更接近隔日可成交波動區間的掛單研究表。這是研究輔助，不自動下單。": {"en": "Converts strategy-level buy / take-profit / stop-loss prices into next-day reachable order ranges. This is research support only and never places trades.", "ja": "戦略レベルの買い / 利確 / 損切り価格を、翌日に約定しやすい値動きレンジへ変換する注文研究表です。研究支援のみで自動注文は行いません。", "ko": "전략 수준의 매수 / 익절 / 손절 가격을 익일 체결 가능성이 있는 주문 구간으로 변환한 연구 표입니다. 연구 보조용이며 자동 주문하지 않습니다."},
    "目前沒有足夠持倉或行情資料產生隔日掛單計畫。請確認 my_stocks.json / my_sotcks.json 與行情資料已載入。": {"en": "Not enough portfolio or market data to build a next-day order plan. Confirm my_stocks.json / my_sotcks.json and market data are loaded.", "ja": "翌日注文計画を作成する持ち株または市場データが不足しています。my_stocks.json / my_sotcks.json と市場データが読み込まれているか確認してください。", "ko": "익일 주문 계획을 만들 포트폴리오 또는 시세 데이터가 부족합니다. my_stocks.json / my_sotcks.json 및 시세 데이터가 로드되었는지 확인하세요."},
    "可規劃標的": {"en": "Plannable tickers", "ja": "計画可能銘柄", "ko": "계획 가능 종목"},
    "高/中買進成交機率": {"en": "High/medium buy reachability", "ja": "高/中 買い到達確率", "ko": "높음/중간 매수 도달 가능성"},
    "高/中賣出成交機率": {"en": "High/medium sell reachability", "ja": "高/中 売り到達確率", "ko": "높음/중간 매도 도달 가능성"},
    "保護性停損單": {"en": "Protective stops", "ja": "保護的損切り", "ko": "보호 손절"},
    "此頁輸出的是隔日限價 / 停損研究計畫，不會連接券商，也不會自動下單。實際掛單前請確認即時報價、盤前盤後價差、流動性與個人風險。": {"en": "This page outputs next-day limit / stop research plans. It does not connect to brokers or place trades. Before placing real orders, confirm live quotes, pre/post-market spreads, liquidity, and personal risk.", "ja": "このページは翌日の指値 / 損切り研究計画を出力します。証券会社へ接続せず、自動注文もしません。実際に発注する前に、リアルタイム価格、時間外スプレッド、流動性、個人リスクを確認してください。", "ko": "이 페이지는 익일 지정가 / 손절 연구 계획을 출력합니다. 브로커에 연결하거나 자동 주문하지 않습니다. 실제 주문 전 실시간 호가, 프리/애프터마켓 스프레드, 유동성, 개인 위험을 확인하세요."},
    "隔日買進區": {"en": "Next-day buy zone", "ja": "翌日買いゾーン", "ko": "익일 매수 구간"},
    "隔日賣出區": {"en": "Next-day sell zone", "ja": "翌日売りゾーン", "ko": "익일 매도 구간"},
    "戰術停損": {"en": "Tactical stop", "ja": "戦術的損切り", "ko": "전술 손절"},
    "硬停損": {"en": "Hard stop", "ja": "ハードストップ", "ko": "하드 스톱"},
    "策略買進價": {"en": "Strategy buy price", "ja": "戦略買い価格", "ko": "전략 매수가"},
    "策略停利價": {"en": "Strategy take-profit price", "ja": "戦略利確価格", "ko": "전략 익절가"},
    "買進成交機率": {"en": "Buy reachability", "ja": "買い到達確率", "ko": "매수 도달 가능성"},
    "賣出成交機率": {"en": "Sell reachability", "ja": "売り到達確率", "ko": "매도 도달 가능성"},
    "戰術停損觸及機率": {"en": "Tactical stop reachability", "ja": "戦術的損切り到達確率", "ko": "전술 손절 도달 가능성"},
    "20日中位日內波動%": {"en": "20D median intraday range %", "ja": "20日中央値 日中レンジ%", "ko": "20일 중앙값 일중 변동폭%"},
    "20日80分位日內波動%": {"en": "20D 80th percentile intraday range %", "ja": "20日80パーセンタイル 日中レンジ%", "ko": "20일 80분위 일중 변동폭%"},
    "建議單型": {"en": "Suggested order type", "ja": "推奨注文タイプ", "ko": "추천 주문 유형"},
    "原因": {"en": "Reason", "ja": "理由", "ko": "이유"},
    "高": {"en": "High", "ja": "高", "ko": "높음"},
    "中": {"en": "Medium", "ja": "中", "ko": "중간"},
    "低到中": {"en": "Low to medium", "ja": "低〜中", "ko": "낮음~중간"},
    "低，偏策略價": {"en": "Low; strategy-level price", "ja": "低：戦略価格寄り", "ko": "낮음; 전략 가격"},
    "買進限價單": {"en": "Buy limit", "ja": "買い指値", "ko": "매수 지정가"},
    "加碼限價單": {"en": "Add limit", "ja": "追加買い指値", "ko": "추가 매수 지정가"},
    "分批停利限價單": {"en": "Staged take-profit limit", "ja": "分割利確指値", "ko": "분할 익절 지정가"},
    "保護性停損單": {"en": "Protective stop", "ja": "保護的損切り", "ko": "보호 손절"},
    "減碼 / 避開": {"en": "Reduce / avoid", "ja": "縮小 / 回避", "ko": "축소 / 회피"},
    "不掛單，等待確認": {"en": "No order; wait for confirmation", "ja": "注文せず確認待ち", "ko": "주문 없음; 확인 대기"},
    "停利 + 停損 OCO 計畫": {"en": "Take-profit + stop OCO plan", "ja": "利確 + 損切り OCO 計画", "ko": "익절 + 손절 OCO 계획"},
    "如何使用隔日掛單計畫？": {"en": "How to use the Next-Day Order Planner", "ja": "翌日注文プランナーの使い方", "ko": "익일 주문 계획 사용법"},
    "下載隔日掛單計畫 CSV": {"en": "Download next-day order plan CSV", "ja": "翌日注文計画CSVをダウンロード", "ko": "익일 주문 계획 CSV 다운로드"},
}
_TRANSLATIONS.update(_NEXT_DAY_ORDER_TRANSLATIONS)
