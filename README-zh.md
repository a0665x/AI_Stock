...
- 今日決策 / Today Dashboard：整合今日機會雷達、市場熱力圖、策略健檢卡與 TradingView 式行動清單；行動清單顯示最終方向、優先分數、買賣區、最終策略與下一步，並說明「等待確認」不是沒有模型，而是目前優勢不足以追價
...
- 研究中心 / Professional Research Center：整合 Research Radar、Strategy Tester、Factor Explorer、Training Data Studio 與 Risk & Correlation Monitor；像 TradingView + 回測平台一樣，先看研究總覽，再看策略回測、因子排行、模型歸因、訓練資料與相關風險；顏色語意與 AUC / Profit Factor / 最大回撤都有白話說明，昂貴分析維持按鈕觸發，不拖慢首頁
...
左側 sidebar 現在只保留全域設定：資料來源、股票代號、歷史區間、K 線週期、決策天數、個人交易偏好、CSV 上傳與重新抓資料。頁面專屬設定都在各自頁內，例如策略驗證、研究中心與圖表分析。

主流程建議：

1. 先看「今日決策」：今天該優先看哪幾檔、最終方向、買賣區與下一步。
2. 到「交易計畫」：把方向轉成明天可研究的掛單區、停損與停利。
3. 到「圖表分析」：像 TradingView 一樣確認 K 線、支撐壓力、SMC、Entry/Stop/Target 是否合理。
4. 到「策略驗證」：檢查策略在該股票近期是否真的有效。
5. 需要研究模型、因子、相關性或輸出 Training Data 時，再到「研究中心」。
...
11. 到「研究中心」先看 Research Radar，再進 Strategy Tester / Factor Explorer / Training Data Studio / Risk & Correlation Monitor。
12. 在 Factor Explorer 比較未來 1/3/5/10 天哪個比較有訊號，並查看最有效因子欄位與 SHAP/fallback 歸因。
13. 在 Training Data Studio 下載分析結果數據（Training Data），檢查哪些欄位和未來 3 天報酬最相關；horizon 在 UI 中改稱「預測幾天後」。
14. 在 Risk & Correlation Monitor 確認多檔股票是否集中在同一類風險。
